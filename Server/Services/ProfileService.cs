...
namespace MiyakoCarryService.Server.Services
{
    ...
    )
    {
        ...
        public void BotDiedCooldown(MongoId botProfileId)
        {
            var cooldownMinutes = configService.GetMcsPluginConfig().ServerConfig.TeamKillCooldownMinutes;
            infoService.SetBotCooldown(botProfileId, cooldownMinutes);
            logger.Info($"[MiyakoCarryService] Companion bot {botProfileId} died in raid, set to recovering for {cooldownMinutes} minutes.");
        }
        ...
        public SptProfile Generate(MongoId mcsLeadPlayerId, MongoId mcsBotPlayerId, PmcData completeQuestPmcData, OrderInfo orderInfo)
        {
            ...
            var side = isPmc
                ? (orderInfo.SpawnType.WildSpawnType == "pmcUSEC" ? "Usec" : orderInfo.SpawnType.WildSpawnType == "pmcBEAR" ? "Bear" : completeQuestPmcData.Info.Side)
                : "Savage";
            var role = isPmc
        ...
        public void EnsureAutoGeneratedBots(MongoId mcsLeadPlayerId, int targetCount = 10)
        {
            if (!MongoId.IsValidMongoId(mcsLeadPlayerId))
            {
                return;
            ...
            if (pmcData is null)
            {
                pmcData = new PmcData
                {
                    ...
                    Info = new()
                    {
                        ...
                        Side = "Usec",
                        Level = 1,
                        RegistrationDate = (int)timeUtil.GetTimeStamp(),
                        GameVersion = GameEditions.EDGE_OF_DARKNESS
                    }
                };
            }
            ...
            // 1. Ensure all existing bots have valid, active orders unless they are currently in cooldown/recovering
            ...
            foreach (var profile in existingProfiles)
            {
                ...
                if (existingOrder == null)
                {
                    var newOrder = new OrderInfo
                    {
                        ...
                        ExpirationTime = currentTime + 3153600000L
                    };
                    infoService.AddOrderInfo(newOrder);
                }
                else if (existingOrder.Status == EInfoStatus.Expired && currentTime < existingOrder.ExpirationTime)
                {
                    // Bot is currently in cooldown/recovering -> Keep it in recovering state!
                    continue;
                }
                else if (existingOrder.Status == EInfoStatus.Expired || existingOrder.Duration >= 999999 || existingOrder.ExpirationTime < currentTime + 86400 * 30)
                {
                    ...
                    existingOrder.ExpirationTime = currentTime + 3153600000L;
                }
            }

            if (existingProfiles.Count > targetCount)
            {
                int excessCount = existingProfiles.Count - targetCount;
                logger.Info($"[MiyakoCarryService] Trimming {excessCount} excess companion bots to match target count ({targetCount}) for player {mcsLeadPlayerId}...");
                for (int i = targetCount; i < existingProfiles.Count; i++)
                {
                    var profileToRemove = existingProfiles[i];
                    var botIdToRemove = profileToRemove?.ProfileInfo?.ProfileId;
                    if (botIdToRemove == null) continue;
            ...
            for (int i = 0; i < countToGenerate; i++)
            {
                ...
                infoService.AddOrderInfo(orderInfo);
                try
                {
                    Generate(mcsLeadPlayerId, botPlayerId, pmcData, orderInfo);
                }
                catch (Exception ex)
                {
                    logger.Error($"[MiyakoCarryService] Failed to auto-generate companion bot #{i + 1} ({spawnType.DisplayName}): {ex.Message}");
                }
            }
        ...
        public void AutoGenerateBotsForLoadedProfiles(int targetCount = 100)
        {
            try
            {
                ...
                if (profiles is not null)
                {
                    foreach (var kvp in profiles)
                    {
                        ...
                        if (!MongoId.IsValidMongoId(playerId))
                        {
                            continue;
                        }
            ...
            {
                logger.Error($"[MiyakoCarryService] Error while auto-generating companion bots on startup: {ex.Message}");
            }
        }
        ...
        public async Task OnPostLoadAsync()
        {
            ...
            var targetCount = configService.GetMcsPluginConfig()?.ServerConfig?.AutoGenerateBotCount ?? 100;
            AutoGenerateBotsForLoadedProfiles(targetCount);
        }
    }
}