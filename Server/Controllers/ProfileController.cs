...
namespace MiyakoCarryService.Server.Controllers
{
    ...
    )
    {
        ...
        {
            profileService.EnsureAutoGeneratedBots(mcsLeadPlayerId, targetCount);
        }

        public void CheckAndUpdateBotsForPlayerLevel(MongoId mcsLeadPlayerId)
        {
            profileService.CheckAndUpdateBotsForPlayerLevel(mcsLeadPlayerId);
        }

        public void AwardRaidXpToCompanionBots(MongoId mcsLeadPlayerId)
        {
            profileService.AwardRaidXpToCompanionBots(mcsLeadPlayerId);
        }

        public void RotateOneCompanionBot(MongoId mcsLeadPlayerId, int count = 1)
        {
            profileService.RotateOneCompanionBot(mcsLeadPlayerId, count);
        }

        public void BotDiedCooldown(MongoId botProfileId)
        {
            profileService.BotDiedCooldown(botProfileId);
        }

        public void AutoGenerateBotsForLoadedProfiles(int targetCount = 100)
        {
            profileService.AutoGenerateBotsForLoadedProfiles(targetCount);
        }
    }
}